...
import (
	...
	"github.com/alexohneander/flotte/scheduling"
	"github.com/alexohneander/flotte/utils"
...
func (s *HTTPServer) PostSchedule(c *CustomContext) error {
	...
	if errors.Is(err, context.DeadlineExceeded) {
		s.mustEmitRelease(c.RequestID, workerRes.WorkerID) // tell the worker to release resources
		return echo.NewHTTPError(http.StatusGatewayTimeout, "worker did not respond in time")
	}
